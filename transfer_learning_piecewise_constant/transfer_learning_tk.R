...
tic()
#Plot 1: configuration 1 and vary hat{t}_{k}
...
result_fl =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_1 =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_ora =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_est =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0 =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_1 =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_ora =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_est =  array(NA, dim = c(length(t_k_list), length(H_k_list), length(A_list),  length(gamma_list), simu))


for (t_k_index in 1:length(t_k_list)){
  t_k = t_k_list[t_k_index]
  for (H_k_index in 1:length(H_k_list)){
    ...
    for (A_index in 1:length(A_list)){
      ...
      for (gamma_index in 1:length(gamma_list)) {
        ...
        set.seed(10*t_k_index*H_k_index*A_index*gamma_index*simu_test)
        ...
        result_fl[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_test-f_0)^2)/n_0
        ...
        result_fl_T_1[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_1_test -f_0)^2)/n_0
        ...
        result_fl_T_ora[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_ora_test -f_0)^2)/n_0
        
        
        
        fit_l0_test = cv_l_0_est(y_0) 
        
        result_l0[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_test-f_0)^2)/n_0
        
        
        fit_l0_T_1_test =  cv_l_0_est(y_1_project)
        
        result_l0_T_1[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_1_test-f_0)^2)/n_0
        
        
        fit_l0_T_ora_test =  cv_l_0_est(y_A_project)
        
        result_l0_T_ora[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_ora_test-f_0)^2)/n_0
        ...
        result_fl_T_est[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_est_test -f_0)^2)/n_0
        
        
        fit_l0_T_est_test =  cv_l_0_est(y_A_est_project)
        
        result_l0_T_est[t_k_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_est_test-f_0)^2)/n_0
...
t_k_list <- rep(c(25, 50, 75, 100), each = 8)

d_1 <- data.frame(t_k_list = t_k_list, Method = Method, value = value_1, se = se_1)
...
plott_1 <- ggplot(data = d_1, mapping = aes(x = t_k_list, y = value, colour = Method)) +
  geom_line() + 
  geom_point() + 
  geom_errorbar(aes(ymin = value - se, ymax = value + se), width = 2.5) +
  labs(y = "Estimation error", x = expression(hat(t)[k])) +
  scale_color_manual(
    values = color_list,
