...
jump.location = c(0, 0.31,  0.36,  0.39,   0.45,  0.49, 0.57,  0.64,   0.77,   0.97)
...
delta <- rep(c(0.25, 0.50, 0.75, 1.00), each = 8)
...
plott_1 <- ggplot(data = d_1, mapping = aes(x = delta, y = value, colour = Method)) +
  ...
  scale_color_manual(
    values = c("#66c2a5", "#fc8d26", "#8da0cb", "#e78ac3", "#484B8A", "#e5c494", "#ffd92f", "#b3b3bc"),
  ...
  for (H_k_index in 1:length(H_k_list)){
    ...
    for (A_index in 1:length(A_list)){
      ...
      for (gamma_index in 1:length(gamma_list)) {
        ...
        jump.mean = c(2*gamma, 4*gamma, 1*gamma, 5*gamma, 7*gamma, 8*gamma, 2*gamma, 1*gamma, 3*gamma, 4*gamma)
        for (simu_test in 1:simu){
          set.seed(100*h_index*H_k_index*A_index*gamma_index*simu_test)
...
h = rep(c(0.50, 1.00, 1.50, 2.00), each = 8)
...
sig.delta_1_list = c(0.5)
sig.delta_2  = 3
H_k_list = c(0.1, 0.2, 0.3, 0.4)
A_list = c(10)
gamma_list = c(0.5)
...
dev.copy2pdf(file=paste("scenario_2_tl.pdf",sep = ''),width = 8,height = 10)
