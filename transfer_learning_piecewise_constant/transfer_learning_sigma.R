...
result_fl =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_1 =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_ora =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_fl_T_est =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0 =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_1 =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_ora =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))
result_l0_T_est =  array(NA, dim = c(length(sigma_list), length(H_k_list), length(A_list),  length(gamma_list), simu))


for (sigma_index in 1:length(sigma_list)){
  ...
  for (H_k_index in 1:length(H_k_list)){
    ...
    for (A_index in 1:length(A_list)){
      ...
      for (gamma_index in 1:length(gamma_list)) {
        ...
        set.seed(10*sigma_index*H_k_index*A_index*gamma_index*simu_test)
        ...
        result_fl[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_test-f_0)^2)/n_0
        ...
        result_fl_T_1[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_1_test -f_0)^2)/n_0
        ...
        result_fl_T_ora[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_ora_test -f_0)^2)/n_0
        
        
        
        fit_l0_test = cv_l_0_est(y_0) 
        
        result_l0[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_test-f_0)^2)/n_0
        
        
        fit_l0_T_1_test =  cv_l_0_est(y_1_project)
        
        result_l0_T_1[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_1_test-f_0)^2)/n_0
        
        
        fit_l0_T_ora_test =  cv_l_0_est(y_A_project)
        
        result_l0_T_ora[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_ora_test-f_0)^2)/n_0
        ...
        result_fl_T_est[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_fl_T_est_test -f_0)^2)/n_0
        
        
        fit_l0_T_est_test =  cv_l_0_est(y_A_est_project)
        
        result_l0_T_est[sigma_index, H_k_index, A_index,gamma_index, simu_test] = sum((fit_l0_T_est_test-f_0)^2)/n_0
...
sigma <- rep(c(0.25, 0.5, 0.75, 1), each = 8)

d_1 <- data.frame(sigma = sigma, Method = Method, value = value_1, se = se_1)

color_list <- setNames(c("#66c2a5", "#fc8d26", "#8da0cb", "#e78ac3", "#484B8A", "#e5c494", "#ffd92f", "#b3b3bc"),
                       levels(d_1$Method))

plott_1 <- ggplot(data = d_1, mapping = aes(x = sigma, y = value, colour = Method)) +
  geom_line() + 
  geom_point() + 
